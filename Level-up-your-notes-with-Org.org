...
In Org each entry is simply a line starting with one or more =*=. To insert a
new headline after the current one type =<comma> h i=.
...
place and then I re-file them into other files later on. Note that I clock time
during the capture and then automatically resume the previously clocked task
when I am done capturing.
...
#+BEGIN_SRC elisp :tangle dot-spacemacs
  ;; Set the org-agenda files
  (setq org-agenda-files
    '("~/src/presentations/Level-up-your-notes-with-Org"))
#+END_SRC
